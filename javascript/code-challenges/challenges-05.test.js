...
const toLastNames = people => {
  let newArray = people.map(folks => folks.firstName +' ' + folks.lastName)
  return newArray;
};
...
const addValues = (arr) => {
  ...
  let newArray = arr.reduce((sum, val) => sum += val, 0);
...
const addPurchases = (arr) => {
  let newArray = arr.reduce((sum, val) => sum += val.purchasePrice, 0);
  return newArray
};
...
const countNumberOfElements = (arr) => {
  let newArray = arr.reduce((sum, val) => val ? sum +1:sum+0, 0);         
  return newArray;
};
...
const returnNames = (arr) => {
  let newArray = arr.reduce((sum, val) => {sum.push(val.name)
    return sum},[]);
    return newArray;
};
